...
namespace SyncStreamAPI.DataContext
{
    public class PostgresContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            builder.Entity<DbConversation>().HasOne(x => x.Sender).WithMany(x => x.StartedConversations).HasForeignKey(x => x.SenderId);
            builder.Entity<DbConversation>().HasOne(x => x.Receiver).WithMany(x => x.ReceivedConversations).HasForeignKey(x => x.ReceiverId);
