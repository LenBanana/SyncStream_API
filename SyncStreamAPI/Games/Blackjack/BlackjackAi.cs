...
public class BlackjackAi
{
        ...
        bool forSplitHand)
    {
        ...
        {
            var aceIdx = forSplitHand
                ? member.splitCards.FindIndex(x => x.Rank == PlayingCardRank.Ace)
                : member.cards.FindIndex(x => x.Rank == PlayingCardRank.Ace);
            if (aceIdx != -1)
                return ReactToSingleAce(
                    forSplitHand ? member.splitCards[aceIdx == 1 ? 0 : 1].Rank : member.cards[aceIdx == 1 ? 0 : 1].Rank,
                    (int)dealerPoints, doubleOption);
        }
