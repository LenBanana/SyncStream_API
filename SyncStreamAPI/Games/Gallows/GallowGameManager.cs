...
public class GallowGameManager
{
    ...
    public static List<GallowLogic> gallowGames { get; set; } = new();
    ...
    public bool PlayNewRound(string UniqueId)
    {
        ...
        if (idx < 0)
        {
            ...
            gallowGames.Add(new GallowLogic(this, UniqueId,
                room.server.members?.Select(x => x.ToGallowMember()).ToList()));
            return true;
        }
    ...
    {
        if (Time % 5 == 0) await _hub.Clients.Group(game.RoomId).gallowusers(game.members);
    ...
    private async void Server_GallowGameEnded(GallowLogic game)
    {
        ...
        await _hub.Clients.Group(game.RoomId).dialog(new Dialog
            { Header = "Gallow Game Ended", Question = playerBoard, Answer1 = "Ok" });
    }
    ...
    public async Task PlayGallow(GallowLogic game, Member sender, ChatMessage message, int Time)
    {
        ...
        if (msg == gallowWord)
        {
            ...
            var correntAnswerPrivateMsg =
                new SystemMessage($"{message.username} you answered correct. You've been awarded {points} points");
            await _hub.Clients.Client(sender.ConnectionId).sendmessage(correntAnswerPrivateMsg);
            if (game.members?.Where(x => !x.isDrawing).All(x => x.guessedGallow) == true) await EndGallow(game, Time);
    ...
    public async Task EndGallow(GallowLogic game, int Time)
    {
        ...
        var idx = game.members.FindIndex(x => x.isDrawing);
        if (idx == -1) await _hub.Clients.Group(game.RoomId).gallowusers(game.members);
        ...
        if (idx > -1)
        {
            ...
            if (hostPoints > 0)
            {
                ...
                var hostMsg =
                    new SystemMessage(
                        $"{game.members[idx].username} {guessedGallow.Count()} users got the word correct, good job. You've been awarded {hostPoints} points");
