...
public class YoutubeApi
{
    ...
    public async Task<YtApi> Search(string query, string nextPageToken = "", int pageSize = 10, string order = "relevance")
    {
        using var httpClient = new HttpClient();
        var url = $"{BaseUrl}search?part=snippet&type=video&maxResults={pageSize}&q={query}&key={_ytApiKey}&order={order}";
