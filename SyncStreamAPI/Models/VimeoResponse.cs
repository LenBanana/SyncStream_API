    ...
    public static VimeoResponse? FromUrl(string url)
    {
        try
        {
            using (var client = new WebClient())
            {
                ...
                var videokey = url.Split('/').Last();
                var json = client.DownloadString("http://vimeo.com/api/v2/video/" + videokey + ".json");
                json = json.Substring(1, json.Length - 2);
                var result = JsonConvert.DeserializeObject<VimeoResponse>(json);
                return result;
            }
        }
        catch (Exception ex)
