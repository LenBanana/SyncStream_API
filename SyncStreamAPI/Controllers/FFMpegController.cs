...
namespace SyncStreamAPI.Controllers
{
    ...
    public class FFMpegController : Controller
    {
        ...
        public async Task<IActionResult> CutMedia(string apiKey, int startMilliSeconds, int endMilliSeconds)
        {
            ...
            var cutFunction = new FFmpegCutMedia(baseFunction.InputPath, baseFunction.OutputPath, startTime, endTime, null, baseFunction.InputFile, baseFunction.OutputFile);
        ...
        public async Task<IActionResult> ConvertMedia(string apiKey, MediaType mediaType)
        {
            ...
            switch (mediaType)
            {
                ...
                case MediaType.AIFF:
                case MediaType.M4A:
                    var convertAudio = new FFmpegConvertAudio(baseFunction.InputPath, baseFunction.OutputPath, baseFunction.InputFile, baseFunction.OutputFile);
                ...
                case MediaType.MOV:
                case MediaType.MKV:
                    var convertVideo = new FFmpegConvertVideo(baseFunction.InputPath, baseFunction.OutputPath, baseFunction.InputFile, baseFunction.OutputFile);
                    return await FFmpegTools.ProcessMedia(inputFile, dbUser, convertVideo, mimeType, _postgres, _hub);
                case MediaType.GIF:
                    var convertGIF = new FFmpegConvertGIF(baseFunction.InputPath, baseFunction.OutputPath, baseFunction.InputFile, baseFunction.OutputFile);
